...
$( document ).on( 'pageshow', 'div[id="articles"]', function(e, data){
    ...
    $.get(categoryURL, function (categorydata) {
        ...
        $("div[class=listing_head]:first", categorydata.responseText).each(function () {
        ...
        $("div[class=list_nav]:first a[href]", categorydata.responseText).each(function () {
        ...
        $("div[class=list_nav]:first", categorydata.responseText).each(function () {
            var navdata = $(this),
        ...
        $("div[class=content_list]:first li", categorydata.responseText).each(function () {
            var newsitemdata = $(this),
                newsitemurl = $("a", newsitemdata).attr('href');
            if (newsitemurl) {
                if (categoryURL.indexOf(newsitemurl) == -1) {
                    articlelist += "<li>";
                    articlelist += "<a href='view.htm?url=" + encodeURIComponent(newsitemurl) + "'>";
                    articlelist += "<p>" + $("a", newsitemdata).text() + "</p>";
                    articlelist += "<p>" + $("span", newsitemdata).text() + "</p>";
                    articlelist += "</a>";
                    articlelist += "</li>";
                }
            }
...
$( document ).on( 'pageshow', 'div[id="view"]', function(e, data){
    ...
    $.get(newsURL, function (newsdata) {
        ...
        $("div[id=tit]:first, td[class=content1_3]:first", newsdata.responseText).each(function () {
        ...
        $("div[id=content]:first", newsdata.responseText).each(function () {
            $(this).find('div').first().remove();
            $(this).find('div[id=social_share]').first().remove();
            $(this).find('div[id=pages_nav]').first().remove();
            var newscontent = $(this).html();
            markup += newscontent; //modifiednewscontent;
            //console.log(newscontent);
        });
        markup += "</div><br /><br />";
        ...
        // and add it to our markup.
        $('div[class=rel] ul li a[href], td[class=content1_6] ul li a[href]', newsdata.responseText).each(function () {
